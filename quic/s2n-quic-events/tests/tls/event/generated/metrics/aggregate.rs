...
impl<R: Registry> Subscriber<R> {
    ...
    pub fn new(registry: R) -> Self {
        let mut counters = Vec::with_capacity(2usize);
        ...
        counters.push(registry.register_counter(&INFO[2usize]));
        {
            ...
            use api::*;
            {
                ...
                for variant in <TestEnum as AsVariant>::VARIANTS.iter() {
                    nominal_counters
                        .push(registry.register_nominal_counter(&INFO[1usize], variant));
                    count += 1;
                }
...
impl<R: Registry> event::Subscriber for Subscriber<R> {
    ...
    fn on_enum_event(
        ...
        self.count_nominal(1usize, 0usize, &event.value);
        let _ = context;
        let _ = meta;
        let _ = event;
    }
