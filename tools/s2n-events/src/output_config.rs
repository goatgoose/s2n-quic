...
impl OutputConfig {
    ...
    fn supervisor_supported(&self) -> bool {
        if let PublisherTarget::C = self.publisher {
            return false;
        }
    ...
    pub fn publisher_trait_imports(&self) -> TokenStream {
        match self.publisher {
            PublisherTarget::C => quote!(),
            PublisherTarget::Rust => quote!(
                use core::fmt;
            ),
        }
    }

    pub fn publisher_traits(
        &self,
        endpoint_publisher: &TokenStream,
        endpoint_publisher_subscriber: &TokenStream,
        connection_publisher: &TokenStream,
        connection_publisher_subscriber: &TokenStream,
    ) -> TokenStream {
        if let PublisherTarget::C = self.publisher {
            return quote!();
        }
    ...
    pub fn snapshot_publisher(
        &self,
        testing_fields: &TokenStream,
        testing_fields_init: &TokenStream,
        endpoint_publisher_testing: &TokenStream,
        connection_publisher_testing: &TokenStream,
    ) -> TokenStream {
        if let PublisherTarget::C = self.publisher {
            return quote!();
        }

        let testing_output_type = self.testing_output_type();
        let lock = self.lock();
